...
chrome.devtools.network.onRequestFinished.addListener(request => {
  request.getContent((body) => {
    ...
    createAndShowPieChart(JSON.parse(body));
    console.log(body);
	}
...
/*Object.values(jsonData).forEach(([chartName, portfolio], index) => {
...
holdings.forEach((holding, i) => {
  ...
  legendLabel.textContent = `${holding.ticker}: ${holding.value.toFixed(2)}`;
  legendItem.appendChild(legendLabel);

  legendContainer.appendChild(legendItem);
});*/

//{"result":{"status":200,"ok":true,"message":"{\"4621c101-e34f-48f0-86c4-4f49ae25648d\" {:available-cash 2226.2800000000000M, :net-liquidity 3023.1933990409658M, :total-cash 2226.2800000000000M, :net-liquidity-broker-perspective 3023.1933990409658M, :total-cash-broker-perspective 2226.2800000000000M, :total-cash-composer-perspective 2149.6300000000000M, :available-cash-composer-perspective 2149.6300000000000M}}"}}
//https://app.composer.trade/getAccountSnapshots

//https://stagehand-production-dot-leverheads-278521.uc.r.appspot.com/api/v1/portfolio/stats
//{"portfolio_value":3023.1933990409658,"todays_dollar_change":212.5220757908738,"todays_percent_change":0.0756125677,"total_cash":2226.28,"simple_return":0.007731,"net_deposits":3000.00,"total_unallocated_cash":2149.63}
...
Object.entries(data).forEach(([chartName, portfolio], index) => {
  const holdings = portfolio.holdings;
  const labels = holdings.map(holding => holding.ticker);
  const values = holdings.map(holding => holding.current_allocation* 100);
  ...
  // Create the canvas element inside the chart container
  const canvas = document.createElement('canvas');
  chartContainer.appendChild(canvas);
